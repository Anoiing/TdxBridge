...
Hermes、OpenClaw 等智能体和自动化工具在 Linux 或 macOS 上通常更容易部署、调试和长期运行；但通达信客户端及其本地量化能力又主要绑定在 Windows 桌面环境里。直接把全部工作都放到 Windows 上，智能体运行体验不佳；把全部工作迁到 Linux/macOS，又拿不到通达信软件能力。

TdxBridge 解决的就是这个“鱼和熊掌不可兼得”的尴尬：让通达信继续运行在 Windows 上，同时把它的能力通过局域网 HTTP 接口稳定暴露给 Linux/macOS 上运行的 Hermes、OpenClaw 或其他调用方。
...
面向 Windows 使用者，建议先到 [GitHub Releases 页面](https://github.com/Anoiing/TdxBridge/releases) 下载已经打包好的 `TdxBridge-release.zip`。解压后，在项目根目录右键管理员运行：
